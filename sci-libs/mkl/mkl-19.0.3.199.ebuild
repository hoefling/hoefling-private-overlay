...
MY_PV='20'$(ver_cut 1)  # 2019
MY_PV2="${MY_PV}."$(ver_cut 3)'-'$(ver_cut 4)  # 2019.3-199
MY_PV3=${MY_PV}.$(ver_cut 3-4)  # 2019.3.199
...
INTEL_DIST_DAT_RPMS=(
	...
	"mkl-common-c-ps-${MY_PV2}-${MY_PV2}.noarch.rpm"
	"mkl-common-f-${MY_PV2}-${MY_PV2}.noarch.rpm"
	"mkl-common-ps-${MY_PV2}-${MY_PV2}.noarch.rpm"
	"mkl-f95-common-${MY_PV2}-${MY_PV2}.noarch.rpm")
INTEL_DIST_AMD64_RPMS=(
	"mkl-cluster-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-cluster-rt-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-core-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-core-c-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-core-f-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-core-ps-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-core-rt-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-f95-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-gnu-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-gnu-c-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-gnu-f-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-gnu-f-rt-${MY_PV2}-${MY_PV2}.x86_64.rpm"
...
INTEL_DIST_X86_RPMS=(
	"mkl-core-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-core-c-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-core-f-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-core-ps-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-core-rt-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-f95-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-gnu-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-gnu-c-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-gnu-f-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-gnu-f-rt-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-gnu-rt-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-tbb-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm"
	"mkl-tbb-rt-32bit-${MY_PV2}-${MY_PV2}.x86_64.rpm")

pkg_setup() {
	if use doc; then
		INTEL_DIST_DAT_RPMS+=(
			"mkl-doc-${MY_PV}-${MY_PV2}.noarch.rpm"
			"mkl-doc-ps-${MY_PV}-${MY_PV2}.noarch.rpm")
	fi
}
...
src_install() {
	...
	sed -i -e "s#<INSTALLDIR>#$(isdp_get-sdp-edir)/linux#" \
		"${ED}"/opt/intel/compilers_and_libraries_${MY_PV3}/linux/mkl/bin/pkgconfig/* \
		|| die "sed failed"

	mkdir -p "${ED}"/usr/$(get_libdir)/pkgconfig/ || die "mkdir failed"
	cp "${ED}"/opt/intel/compilers_and_libraries_${MY_PV3}/linux/mkl/bin/pkgconfig/* \
